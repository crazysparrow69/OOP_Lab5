...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
        ...
        break;
    case WM_LBUTTONUP:
        table.addString(editor.OnLBup(hWnd));
        break;
    case WM_MOUSEMOVE:
        editor.OnMouseMove(hWnd);
        break;
    case WM_PAINT:
        editor.rmItem(table.getRemove() - 1);
        table.setRemove(0);
    ...
    case WM_COMMAND:
        {
            ...
            case ID_TOOL_POINT:
            case IDM_POINT:
                editor.Start(new PointShape, hWnd, wmId);
                break;

            case ID_TOOL_LINE:
            case IDM_LINE:
                editor.Start(new LineShape, hWnd, wmId);
                break;

            case ID_TOOL_RECT:
            case IDM_RECT:
                editor.Start(new RectShape, hWnd, wmId);
                break;

            case ID_TOOL_ELLIPSE:
            case IDM_ELLIPSE:
                editor.Start(new EllipseShape, hWnd, wmId);
                break;

            case ID_TOOL_SUPERLINE:
            case IDM_SUPERLINE:
                editor.Start(new SuperlineShape, hWnd, wmId);
                break;

            case ID_TOOL_CUBE:
            case IDM_CUBE:
                editor.Start(new CubeShape, hWnd, wmId);
                break;
